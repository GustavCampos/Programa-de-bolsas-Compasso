...
SELECT 
	email, 
	income, 
	ROUND((income / 1200), 2) AS "salários mínimos" 
...
SELECT 
	...
	income, 
	ROUND((income / 1200), 2) AS "salários mínimos",
	(ROUND((income / 1200), 2) > 4) AS "acima de 4 salários" 
FROM sales.customers c;
...
SELECT 
	email, 
	income, 
	ROUND((income / 1200), 2) AS "salarios minimos",
	(ROUND((income / 1200), 2) > 4) AS "acima de 4 salários" 
FROM sales.customers c
...
SELECT email, city, state FROM sales.customers c
...
SELECT city FROM temp_tables.regions r
WHERE city ILIKE 'z%';
